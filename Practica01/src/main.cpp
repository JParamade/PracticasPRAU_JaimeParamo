...
int main() {
  ...
  while (!glfwWindowShouldClose(pCurrentWindow)) {
    ...
    if (glfwGetKey(pCurrentWindow, GLFW_KEY_UP) == GLFW_PRESS && !bKeyUpPressed) { 
      bKeyUpPressed = true; 
      fPitch += 0.1f; 
      oAudioSource.SetPitch(fPitch); 
    }
    if (glfwGetKey(pCurrentWindow, GLFW_KEY_UP) == GLFW_RELEASE) bKeyUpPressed = false;
    // Decrease source pitch when DOWN key is pressed.
    if (glfwGetKey(pCurrentWindow, GLFW_KEY_DOWN) == GLFW_PRESS && !bKeyDownPressed) { 
    ...
    if (glfwGetKey(pCurrentWindow, GLFW_KEY_LEFT) == GLFW_PRESS) { 
      fPosX -= 0.005f; 
      oAudioSource.SetPosition(fPosX, 0, 0); 
    }
